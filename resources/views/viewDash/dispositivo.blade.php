...
@section('content')
        ...
        <div id="content-wrapper" class="d-flex flex-column" style="background-color:#081f2a">
            ...
            <div id="content">
                ...
                <div class="container-fluid py-4">
                    ...
                    <button type="button" class="btn btn-success d-block mb-4" data-bs-toggle="modal"
                        ...
                        @forelse ($dispositivos as $dispositivo)
                            <div class="col-lg-4 col-md-6" style="margin-bottom: 1.5rem;">
                                <div class="card device-card bg-dark text-white border-0 shadow-lg h-100 rounded-4"
                                    style="border: 2px solid #169b6b !important;">
                                    <div class="card-body p-4">
                                        ...
                                        <div class="mb-3">
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <span class="small mb-0 text-white">
                                                    {{ number_format($dispositivo->consumo_hoje ?? 0, 2) }} kWh
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <span class="small mb-0 text-white">
                                                    {{ number_format($dispositivo->tempo_ligado ?? 0, 1) }}h
                                                </span>
                                            </div>
                                        </div>
                ...
                <div class="modal fade " id="modalAdicionarDispositivo" tabindex="-1"
                    aria-labelledby="modalAdicionarDispositivoLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-lg ">
                        <div class="modal-content border-0 shadow-lg rounded-4" style="background-color:#0f162b">
                            <div class="modal-header border-0 pb-0 px-5 pt-4">
                                <h5 class="modal-title fw-bold" id="modalAdicionarDispositivoLabel">Adicionar Novo
                                    Dispositivo</h5>
                            ...
                            <div class="modal-body px-5 pb-5">
                                <form method="POST" action="{{ route('devices.store') }}" id="formAdicionarDispositivo">
                                    ...
                                    <div class="row g-4">
                                        <div class="col-md-12">
                                            <label for="nome" class="form-label text-white fw-medium">Nome do
                                                dispositivo</label>
                                            <input type="text" name="nome" id="nome"
                                                class="form-control bg-white  border-0"
                                                placeholder="ex: Lâmpada Sala" required autofocus style="background-color: red">
                                            @error('nome')
                                        ...
                                        <div class="col-md-12 mt-4 mb-4">
                                            ...
                                            <select name="tipo" id="tipo" class="form-select  border-0 form-control"
                                                required>
                                        ...
                                        <div class="col-12">
                                            <label for="consumo_base" class="form-label text-white fw-medium">Consumo base
                                                (Watts)</label>
                                            <input type="number" name="consumo_base" id="consumo_base"
                                                class="form-control bg-white text-dark border-0" placeholder="ex: 12"
                                                min="1" required>
                                            @error('consumo_base')
                                                <div class="text-danger small mt-1">{{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="col-12 mt-4">
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox" name="ativo"
                                                    id="ativo" value="1" checked>
                                                <label class="form-check-label text-white" for="ativo">Ativar
                                                    dispositivo imediatamente</label>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            console.log('Toggle script carregado');

            document.querySelectorAll('.device-toggle').forEach(checkbox => {
                checkbox.addEventListener('change', async function(e) {
                    const deviceId = this.getAttribute('data-device-id');
                    const originalChecked = this.checked;
                    this.checked = originalChecked; // mantém visual imediato
                    const badge = document.getElementById('status-' + deviceId);
                    if (badge) {
                        badge.textContent = originalChecked ? 'Ativo' : 'Inativo';
                        badge.className = 'badge ' + (originalChecked ? 'bg-success' :
                            'bg-secondary');
                    }
                    console.log(
                        `Toggle clicado - ID: ${deviceId}, Novo estado: ${originalChecked}`);
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            if (badge) {
                                ...
                                badge.className = 'badge ' + (data.ativo ? 'bg-success' :
                                    'bg-secondary');
                                console.log('Badge atualizado');
                            } else {
                                console.warn('Badge não encontrado para ID:', deviceId);
                            }

                            // Força atualização do CHECKBOX
                            this.checked = data.ativo;
                            console.log('Checkbox atualizado para:', data.ativo);

                            // Atualiza contadores (se existirem)
                            const ligEl = document.querySelector('.ligados-count');
                            const desEl = document.querySelector('.desligados-count');
                            if (ligEl && desEl) {
                                let lig = parseInt(ligEl.textContent) || 0;
                                let des = parseInt(desEl.textContent) || 0;
                                if (data.ativo) {
                                    ligEl.textContent = lig + 1;
                                    desEl.textContent = des - 1;
                                } else {
                                    ligEl.textContent = lig - 1;
                                    desEl.textContent = des + 1;
                                }
